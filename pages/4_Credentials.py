...
st.caption("Add your projects, certifications, internships and backlogs with domain tags.")
st.markdown("---")

st.info("💡 Domain relevance of each credential is matched against your target role — a Python project boosts Data Science readiness more than an unrelated one.")

tab1, tab2, tab3, tab4 = st.tabs(["📁 Projects", "📜 Certifications", "🏢 Internships", "⚠️ Backlogs"])
...
with tab1:
    ...
    for i, proj in enumerate(st.session_state.projects):
        ...
        st.session_state.projects[i]["title"]  = c1.text_input("Title",  value=proj["title"],  key=f"pt_{i}", placeholder="Project title",  label_visibility="collapsed")
...
with tab2:
    ...
    for i, cert in enumerate(st.session_state.certifications):
        c1, c2, c3 = st.columns([3, 2, 0.5])
        st.session_state.certifications[i]["name"]   = c1.text_input("Name",  value=cert["name"],  key=f"cn_{i}", placeholder="Certification name", label_visibility="collapsed")
...
with tab3:
    ...
    for i, intern in enumerate(st.session_state.internships):
        ...
        st.session_state.internships[i]["company"] = c1.text_input("Company", value=intern["company"], key=f"ic_{i}", placeholder="Company", label_visibility="collapsed")
...
with tab4:
    ...
    if not st.session_state.backlogs:
        st.info("No backlogs added — great!")

    for i, bl in enumerate(st.session_state.backlogs):
        c1, c2, c3 = st.columns([3, 2, 0.5])
        st.session_state.backlogs[i]["subject"] = c1.text_input("Subject", value=bl["subject"], key=f"bs_{i}", placeholder="Subject name", label_visibility="collapsed")
    ...
    if st.button("＋ Add Backlog", key="add_backlog"):
        st.session_state.backlogs.append({"subject": "", "domain": "General / Other"}); st.rerun()

st.markdown("---")

if st.button("🔍 Save & Go to Gap Analysis →", use_container_width=True):
    ...
    st.success("✅ Credentials saved! Go to Gap Analysis from the sidebar.")